    ...
    <div class="container-fluid">
        ...
        <div class="card header-card shadow-sm border-0 mb-4 rounded-4">
            <div class="card-body d-flex flex-wrap justify-content-between align-items-center p-4">
                <div>
                    <h4 class="fw-bold m-0 text-uppercase" style="color: var(--navy-theme); letter-spacing: 0.5px;">
                        <i class="fas fa-pray me-2 text-primary"></i>
                        Spiritual Tracker
                    </h4>

                    <p class="text-muted small m-0 mt-1">
                        Input Amalan Yaumiyah Bulanan Berbasis Spreadsheet
                    </p>

                    <div class="mt-2">
                        <span class="badge badge-progress-total p-2 rounded-3 fs-6 shadow-sm">
                            <i class="fas fa-chart-line me-1 text-info"></i>
                            Total Progress Bulan Ini:
                            <span id="grand-total-pct">0.0%</span>
                        </span>
                    </div>
                </div>

                <form class="d-flex gap-2 mt-3 mt-md-0" method="GET" action="{{ route('amalan.index') }}">
            ...
            <table class="table table-amalan align-middle m-0">
                ...
                <tbody>
                    @foreach ($list_amalan as $key => $attr)
                        ...
                        <tr>
                            <td class="sticky-col-1 fw-bold text-dark">
                                {{ $attr['nama'] }}
                            </td>
                            ...
                            @for ($d = 1; $d <= $jumlah_hari; $d++)
                                ...
                                <td class="p-1 text-center border-end border-light">
                                    @if ($key === 'shalat_5_waktu')
                                        <div class="d-flex justify-content-center align-items-center gap-1 px-1 shalat-group" style="min-width: 95px;">
                                            @php
                                                $sh_label = ['S', 'D', 'A', 'M', 'I'];
                            ...
                            @php
                                if ($attr['tipe'] === 'harian') {
                                    $persen = ($total_input / ($jumlah_hari * $attr['target'])) * 100;
                                } else {
                                    $persen = ($total_input / $attr['target']) * 100;
                                }
        ...
        <div id="notif" class="alert alert-success alert-saved shadow-lg py-2 px-4 border-0 text-white bg-success" style="display: none;">
            <i class="fas fa-check-circle me-2"></i>
            Data amalan berhasil diperbarui!
        </div>
...
$(document).ready(function () {
    ...
    const totalJenisAmalan = {{ count($list_amalan) }};
    ...
    $('.shalat-mini-check').on('change', function () {
        ...
        const tgl = input.data('tgl');
        const kolom = input.data('kolom');
        const group = input.closest('.shalat-group').find('.shalat-mini-check');
    ...
    function updatePercentage(kolom) {
        ...
        if (kolom === 'shalat_5_waktu') {
            $('.shalat-group').each(function () {
                $(this).find('.shalat-mini-check').each(function () {
        ...
        if (tipe === 'harian') {
            persen = (total / (jumlahHari * target)) * 100;
        } else {
        ...
        $(`#pct-${kolom}`).text(persen.toFixed(1) + '%');
    ...
    function recalcGrandTotal() {
        ...
        $('.percent-badge').each(function () {
            totalPersen += parseFloat($(this).text()) || 0;
        });

        const grandTotal = totalPersen / totalJenisAmalan;
    ...
    const searchAwardee = document.getElementById('searchAwardee');

    if (searchAwardee) {
        searchAwardee.addEventListener('keyup', function () {
